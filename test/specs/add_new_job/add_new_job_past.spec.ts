...
describe('Create new job - End date in the past', () => {
    before(async () => {
        await login();
    });
    ...
    it('Select «Start date» to the date in the past', async () => {
        const currentDate = await addJobPage.getStartDate();
        await addJobPage.selectStartDateInPast(job, currentDate);
        expect(await addJobPage.isStartDateSelected(job.startDate)).true;
    });

    it('Select «End date» to the date in the past', async () => {
        const currentDate = await addJobPage.getEndDate();
        await addJobPage.selectEndDateInPast(job, currentDate);
        expect(await addJobPage.isEndDateSelected(job.endDate)).true;
    });

    it('Enable the «All day» switch', async () => {
        await addJobPage.enableAllDaySwitch();
        expect(await addJobPage.isAllDaySwitched()).true;
        expect(await addJobPage.isSelectStartTimeButtonDisplayed()).false;
        expect(await addJobPage.isSelectEndTimeButtonDisplayed()).false;
    });
    ...
    it('Click «OK» button', async () => {
        ...
        expect(await jobsPage.scrollDownUntilOverdueJobIsDisplayed(job.name)).true;
    });
